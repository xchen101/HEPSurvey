...
Tool31 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D4, `p8q3 [1]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p1 <- ggplot(Tool31, aes(x = `p8q3 [1]`, y = freq, color = D4)) + # <- change
  geom_line() +
  geom_point() +
...
Tool32 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D4, `p8q3 [2]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p2 <- ggplot(Tool32, aes(x = `p8q3 [2]`, y = freq, color = D4)) + # <- change
  geom_line() +
  geom_point() +
...
Tool33 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D4, `p8q3 [3]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p3 <- ggplot(Tool33, aes(x = `p8q3 [3]`, y = freq, color = D4)) + # <- change
  geom_line() +
  geom_point() +
  ggtitle("Ease of use") +
  labs(x = "Not at all affect - Affect a lot", y = "Frequency")
...
Tool34 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D4, `p8q3 [4]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p4 <- ggplot(Tool34, aes(x = `p8q3 [4]`, y = freq, color = D4)) + # <- change
  geom_line() +
  geom_point() +
