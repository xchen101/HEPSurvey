...
Doc1 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D3, `p6q1 [1]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p1 <- ggplot(Doc1, aes(x = `p6q1 [1]`, y = freq, color = D3)) + # <- change
  geom_line() +
  geom_point() +
...
Doc2 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D3, `p6q1 [2]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p2 <- ggplot(Doc2, aes(x = `p6q1 [2]`, y = freq, color = D3)) + # <- change
  geom_line() +
  geom_point() +
...
Doc3 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D3, `p6q1 [3]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p3 <- ggplot(Doc3, aes(x = `p6q1 [3]`, y = freq, color = D3)) + # <- change
  geom_line() +
  geom_point() +
...
Doc4 <- coded_data %>% #<- change
  filter(!is.na(Sex)) %>%
  group_by(D3, `p6q1 [4]`) %>% #<- change  
  summarize(count = n()) %>%
  mutate(freq = count / sum(count))

p4 <- ggplot(Doc4, aes(x = `p6q1 [4]`, y = freq, color = D3)) + # <- change
  geom_line() +
  geom_point() +
